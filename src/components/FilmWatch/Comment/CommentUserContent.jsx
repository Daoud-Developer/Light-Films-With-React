...
const CommentUserContent = ({ commentData, sortType, commentLimit, media_type, id, role, }) => {
    ...
    const sortComment = (commentData, type) => {
        ...
        if (type === "popular") {
            return commentData.docs
                .slice()
                .sort((a, b) => Object.values(b.data()?.reactions).length -
    ...
    const determineReactionText = (reactions) => {
        if (!Object.keys(reactions).includes(currentUser.uid)) {
            return "Reaction";
        }
        // @ts-ignore
    ...
    const removeReaction = (docData, commentId) => {
        ...
        const updatedReactionUserObj = filteredReactionUsers.reduce((acc, current) => ({
            ...acc,
            [current[0]]: current[1],
        }), {});
        updateDoc(doc(db, `${media_type}-${id}`, commentId), {
    ...
    ref={parent}>
      {sortComment(commentData, sortType)
            ...
            return (<Fragment key={doc.id}>
              {!commentHiden.includes(doc.id) && (<li className="mb-6 flex md:gap-4 gap-2 items-start last:mb-0">
                  <div className="w-[44px] h-[44px] shrink-0">
                    <LazyLoadImage src={docData.user.photoURL} alt="" effect="opacity" className="w-11 h-11 rounded-full object-cover " referrerPolicy="no-referrer"/>
                  </div>
                  <div className={`peer ${editingCommentFor === doc.id && "flex-1"}`}>
                    <div className={`relative bg-dark-lighten px-4 py-2 rounded-2xl ${editingCommentFor === doc.id ? "block" : "inline-block"}`}>
                      ...
                      {editingCommentFor !== doc.id && (<p style={{ wordWrap: "break-word" }} className="text-lg mt-1 max-w-[63vw] md:max-w-none">
                          {docData.value}
                        </p>)}
                      {editingCommentFor === doc.id && (<>
                          <form onSubmit={(e) => {
                            ...
                            if (e.key === "Escape")
                                setEditingCommentFor(undefined);
                        }} ref={editValueRef} defaultValue={docData.value} type="text" className="w-full bg-dark-lighten-2 outline-none py-1 px-2 rounded-md mt-1 text-lg text-white" autoFocus/>
                            <button type="submit">
                      ...
                      {currentUser && (<div className="relative group">
                          <>
                            ...
                            <button onClick={() => removeReaction(docData, doc.id)}>
                              {determineReactionText(docData.reactions) ===
                            "Like" && (<p className="text-primary">
                                  {determineReactionText(docData.reactions)}
                                </p>)}
                              {determineReactionText(docData.reactions) ===
                            "Love" && (<p className="text-red-500">
                                  {determineReactionText(docData.reactions)}
                                </p>)}
                              {determineReactionText(docData.reactions) ===
                            "Haha" && (<p className="text-yellow-500">
                                  {determineReactionText(docData.reactions)}
                                </p>)}
                              {determineReactionText(docData.reactions) ===
                            "Wow" && (<p className="text-green-500">
                                  {determineReactionText(docData.reactions)}
                                </p>)}
                              {determineReactionText(docData.reactions) ===
                            "Sad" && (<p className="text-purple-500">
                                  {determineReactionText(docData.reactions)}
                                </p>)}
                              {determineReactionText(docData.reactions) ===
                            "Angry" && (<p className="text-orange-500">
                                  {determineReactionText(docData.reactions)}
                                </p>)}
                            </button>
                          </>
                          ...
                          <div className="group-hover:opacity-100 group-hover:visible opacity-0 invisible bg-dark-lighten-2 transition duration-300 shadow-md px-2 py-2 rounded-full absolute -top-8 -right-[105px] flex gap-2 z-40">
                            <button onClick={() => addReaction(doc.id, "like")}>
                              <AiTwotoneLike className="text-blue-500 hover:scale-125 transition duration-300" size={20}/>
                            </button>
                            ...
                            <button onClick={() => addReaction(doc.id, "haha")}>
                              <BsEmojiLaughingFill className="text-yellow-500 hover:scale-125 transition duration-300" size={20}/>
                            </button>
                            <button onClick={() => addReaction(doc.id, "wow")}>
                              <FaSurprise className="text-green-500 hover:scale-125 transition duration-300" size={20}/>
                            </button>
                            <button onClick={() => addReaction(doc.id, "sad")}>
                              <FaSadTear className="text-purple-500 hover:scale-125 transition duration-300" size={20}/>
                            </button>
                  ...
                  <EditComment setEditingCommentFor={setEditingCommentFor} media_type={media_type} id={id} singleDoc={doc} showOptionFor={showOptionFor} setShowOptionFor={setShowOptionFor} setCommentHiden={setCommentHiden}/>
                </li>)}
            </Fragment>);
        })}
    </ul>);
