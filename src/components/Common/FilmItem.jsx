...
const FilmItem = ({ item }) => {
    return (<Link to={item.media_type === "movie"
            ? `/movie/${item.id}`
            : item.media_type === "tv"
                ? `/tv/${item.id}`
                : `/`}>
      <div className="shadow-sm bg-dark-darken pb-2 rounded-md overflow-hidden hover:scale-105 hover:brightness-110 transition duration-300 relative group">
        <LazyLoadImage alt="Poster film" src={item.media_type === "person"
            ? resizeImage(item.profile_path || "", "w342")
            : resizeImage(item.poster_path, "w342")} className="object-cover" effect="blur"/>
