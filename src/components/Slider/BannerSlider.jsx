...
const BannerSlider = ({ films, dataDetail, isLoadingBanner, }) => {
    ...
    return (<div className="mt-6 relative h-0 md:pb-[45%] pb-[55%]  tw-banner-slider">
      {isLoadingBanner ? (<Skeleton className="absolute top-0 left-0 w-full h-full rounded-lg"/>) : (<Swiper modules={[Navigation, Autoplay]} navigation autoplay={{ delay: 5000, disableOnInteraction: false }} slidesPerView={1} className="!absolute !top-0 !left-0 !w-full !h-full  !rounded-lg">
          {films.map((film, index) => (<SwiperSlide key={film.id}>
              <Link to={film.media_type === "movie"
                    ? `/movie/${film.id}`
                    : `/tv/${film.id}`} className="group">
                  ...
                  <div className="">
                    ...
                    {!isMobile && (<>
                        <div className="flex gap-2 flex-wrap mt-5">
                          {dataDetail[index].genre.map((genre) => (<div className="px-3 py-1 border rounded-full " key={genre.id}>
                              {genre.name}
                            </div>))}
                        </div>
