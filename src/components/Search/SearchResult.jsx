...
const SearchResult = ({ currentTab, query, page, }) => {
    const { data, error, isPreviousData } = useQuery(["search-result", currentTab, query, page], () => getSearchResult(currentTab, query, page), {
    ...
    return (<div className="md:mt-32 mt-7 px-[2vw]">
      <p className="text-white md:text-xl text-lg mb-6">
        Search results for "{query}" ({data?.total_results} results found)
      </p>
      {data && data.results.length === 0 && (<div className="flex flex-col items-center mb-12">
      ...
      <ul className="grid grid-cols-sm md:grid-cols-lg gap-x-8 gap-y-10">
        {data &&
            data.results.map((item) => (<li key={item.id}>
              <FilmItem item={item}/>
            </li>))}
        {!data &&
