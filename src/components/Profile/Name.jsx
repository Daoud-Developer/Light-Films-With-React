...
const Name = ({ setIsUpdating }) => {
    ...
    return (<>
      ...
      <div>
        ...
        {!isUpdatingName && (<div className="flex justify-between mt-1">
        ...
        {isUpdatingName && (<>
            <form onSubmit={changeName} className="flex justify-between mt-1">
              <div className="flex gap-3">
                <input type="text" ref={firstNameValueRef} autoFocus placeholder="First name" onKeyDown={(e) => {
                if (e.key === "Escape")
                    setIsUpdatingName(false);
            }} className="outline-none bg-dark-lighten rounded-md py-1 px-2 w-full"/>
                <input type="text" ref={lastNameValueRef} placeholder="Last name" onKeyDown={(e) => {
                if (e.key === "Escape")
                    setIsUpdatingName(false);
            }} className="outline-none bg-dark-lighten rounded-md py-1 px-2 w-full"/>
              </div>
