...
const EmailVerification = ({ setIsUpdating, }) => {
    ...
    const sendVerificationEmail = () => {
        ...
        sendEmailVerification(firebaseUser)
            ...
            .catch((error) => toast.error(convertErrorCodeToMessage(error.code), {
            position: "top-right",
            autoClose: 2000,
    ...
    return (<>
      ...
      {isVerificationEmailSent && (<>
          <div className="px-5 py-3 rounded-md z-10 bg-dark-lighten-2 md:w-[500px] fixed top-[35%] md:left-[35%] left-[5%] right-[5%] min-h-[150px]">
            ...
            <button onClick={() => setIsVerificationEmailSent(false)} className="px-6 py-1 bg-dark-lighten rounded-full mt-7 tw-absolute-center-horizontal hover:brightness-75 transition duration-300">
              OK
            </button>
          </div>
          <div onClick={() => setIsVerificationEmailSent(false)} className="fixed top-0 left-0 w-full h-full z-[5] bg-black/60"></div>
        </>)}
      <div className="mt-10 flex justify-between max-w-[600px]">
        ...
        {isShowSentButton && (<button onClick={sendVerificationEmail} className="text-primary underline text-lg">
