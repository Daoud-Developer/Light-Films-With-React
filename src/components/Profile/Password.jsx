...
const Password = ({ setIsShowPromptReAuthFor, isUpdatedPassword, setIsUpdatedPassword, newPasswordValueRef, }) => {
    return (<>
      <ToastContainer />
      ...
      {isUpdatedPassword && (<>
          <div className="px-5 py-3 rounded-md z-10 bg-dark-lighten-2 md:w-[350px] fixed top-[35%] md:left-[35%] left-[5%] right-[5%] min-h-[100px]">
            ...
            <button onClick={() => setIsUpdatedPassword(false)} className="px-6 py-1 bg-dark-lighten rounded-full mt-3 tw-absolute-center-horizontal hover:brightness-75 transition duration-300">
              OK
            </button>
          </div>
          <div onClick={() => setIsUpdatedPassword(false)} className="fixed top-0 left-0 w-full h-full z-[5] bg-black/60"></div>
        </>)}
      <div className="mt-10 max-w-[600px]">
        ...
        <form onSubmit={(e) => {
          ...
          <div className="flex-1">
            <input ref={newPasswordValueRef} type="password" className="bg-dark-lighten py-3 rounded-md  outline-none px-5 text-white w-full" placeholder="New password"/>
          </div>
